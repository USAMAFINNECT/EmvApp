...
import com.topwise.sdk.emv.impl.PayDataUtil.CallbackSort;
...
//    private DBManager db = DBManager.getInstance();
    ...
    public boolean importAmount(String amt) {
        ...
        if (amt != null && mEmvTransData != null &&
            ...
            mAmount =String.format("%012d",Long.parseLong(amt));
    ...
    public boolean importOnlineResp(boolean onlineRes, String respCode, String icc55) {
        ...
        if (mCallbackSort == CallbackSort.REQUEST_ONLINE) {
            ...
            }else if(cardPayType == PayDataUtil.KERNTYPE_AMEX){
                return new AMexPayProcess().scriptProcess(onlineRes, respCode, icc55, mResultListener);
            }
        }
        return false;
    }
    ...
    public String getImportAmt() {
        SDKLog.d(TAG, "getImportAmt: " + mAmount);
        return mAmount;
    }
    ...
    private int buildCandidate() throws RemoteException {
        ...
        //TO DO 货币代码需要设置内核数据
        byte[] transCurCode = BytesUtil.hexString2Bytes("0156");
    ...
    private void initData() throws RemoteException {
        ...
        mAmount = null;
         ...
         //List<EmvAidParam> mList = mEmvTransData.getEmvAidList();
