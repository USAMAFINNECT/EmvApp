...
public class QpbocProcess extends BasePayProcess {
    ...
    private AidlQpboc aidlQpboc = EmvDeviceManager.getInstance().getL2Qpboc();
    ...
    boolean scriptProcess(boolean onlineRes, String respCode, String icc55, TransResultListener listener)  {
        SDKLog.d(TAG, "scriptProcess(Qpboc)");
        Bundle param = new Bundle();
        if (PayDataUtil.AUTH_RES_CODE.equals(respCode) && onlineRes) {
            param.putInt(PayDataUtil.CardCode.TRANS_RESULT, PayDataUtil.CardCode.TRANS_APPROVAL);
            listener.nextTransStep(PayDataUtil.CallbackSort.ON_TRANS_RESULT, param);
        } else {
            param.putInt(PayDataUtil.CardCode.TRANS_RESULT, PayDataUtil.CardCode.TRANS_REFUSE);
            listener.nextTransStep(PayDataUtil.CallbackSort.ON_TRANS_RESULT, param);
        }
        return true;
    }
    ...
    public static int setTLVDataList(byte[] pTLVDatas, int iDataLen) {
        ...
        AidlQpboc aidlQpboc = EmvDeviceManager.getInstance().getL2Qpboc();
