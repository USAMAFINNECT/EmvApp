...
import com.topwise.sdk.emv.EmvDefinition;
...
//    private DBManager db = DBManager.getInstance();
    ...
    private EmvCallback m_emvCallback = new EmvCallback.Stub() {
        ...
        public int cGetOnlinePin(boolean b, byte[] bytes, int i, boolean[] booleans) throws RemoteException {
            ...
            countDownLatchNew();
            emvListener.requestImportPin(PayDataUtil.PINTYPE_ONLINE, false, importFixedAmount);
            countDownLatchAwait();
...
//        emvL2.EMV_AddAIDList(pbocaid1, (byte) pbocaid1.length, (byte) 1);
       ...
       // List<EmvAidParam> mList = emvAidParams;
        List<uAid> mList = AidParam.getDbEmvAidParamList();
...
//        }
    ...
    private int retrieveCAPK() throws RemoteException {
      ...
      //  Capk capk = db.getCapkDao().findByRidIndex(strRid, index[0]);
        String ridindex = new String(BytesUtil.bytes2HexString(rid));
       ...
       //         .append(Integer.toHexString(index[0] & 0xFF)).toString().toUpperCase();
        uCapk capk = CapkParam.queryByRid(BytesUtil.bytes2HexString(rid),BytesUtil.bytes2HexString(index));

        if (null == capk) {
            Log.d(TAG, "findByRidIndex failed!");
            return 0;
        }
        if (ridindex.equals(capk.getRid())){

            Log.d(TAG, "getRid(): " + capk.getRid());
            Log.d(TAG, "getIndex(): " + capk.getRindex());
            Log.d(TAG, "getArithInd(): " + capk.getArithInd());
            Log.d(TAG, "getHashInd(): " + capk.getHashInd());
            Log.d(TAG, "getExponent(): " + capk.getExponent());
            Log.d(TAG, "getModul(): " + capk.getModul());
            Log.d(TAG, "getExpDate(): " + capk.getExpDate());
            Log.d(TAG, "getCheckSum(): " + capk.getCheckSum());
            ...
            emvCapk.setKeyID(Integer.parseInt(capk.getRindex()));
            emvCapk.setArithInd(capk.getArithInd());
            emvCapk.setHashInd(capk.getHashInd());
            emvCapk.setExponent(BytesUtil.hexString2Bytes(capk.getExponent()));
            emvCapk.setModul(BytesUtil.hexString2Bytes(capk.getModul()));
